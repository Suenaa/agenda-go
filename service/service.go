...
import (
	"github.com/Andiedie/agenda-go/model"
)
...
//AddParticipator 添加自己发起的某一会议的一个参与者
func AddParticipator(title string, paticipator string) error {
	return nil
}

//DeleteParticipator 删除自己创建的某一会议的一个参与者
//如果会议的参与者因此变成0，则删除该会议
func DeleteParticipator(title string, participator string) error {
	return nil
}

//QueryMeeting 查询会议，通过开始时间和结束时间查询当前用户需要参加的所有会议
func QueryMeeting(startDate string, endDate string) []model.Meeting {
...
//DeleteAllMeeting 清空当前用户发起的所有会议安排
